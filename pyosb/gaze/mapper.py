...
p_pitch = 0.0025                                      # 2.5 micro meter
phi_cam = -17.0
theta_cam = 0.0
kappa_cam = 0.0
...
def calc_centers(glints, pupils, eye_R, eye_K):
    ...
    eta = (eta0 + eta1) / 2
    iota = n2/n1 * ((np.dot(nu, eta)
                     - np.sqrt((n1/n2)**2 - 1 + np.dot(nu, eta)**2))
                    * nu - eta)
    rci = np.dot(r - c, iota)
...
def calc_gaze(eye_R, eye_K, eye_alpha, eye_beta):
    ...
    for i in tqdm(range(len(ppos[0])), ncols=80):
        pi, ci = calc_centers([glints[0][0][i], glints[0][1][i]],
                              ppos[0][i], eye_R, eye_K)
        # pi, ci = calc_centers([glints[1][0][i], glints[1][1][i]],
